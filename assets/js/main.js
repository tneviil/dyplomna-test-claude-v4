...
(() => {
  ...
  $$('[data-fq]').forEach((sec) => {
    ...
    g.matchMedia().add('(min-width: 901px) and (prefers-reduced-motion: no-preference)', () => {
      ...
      const STEP = .5;                                 // how closely one card follows the next
      const span = () => window.innerHeight * .52 * (STEP * cards.length + 2.4);
      const tl = g.timeline({
        // a slightly longer scrub lets the cards glide instead of snapping to every wheel tick
        scrollTrigger: { trigger: sec, start: 'top top', end: () => '+=' + span(), pin: pin, scrub: 1.6, invalidateOnRefresh: true },
      });
      ...
      // the top of the arc, just above the heading - but never so high that a card slides under the fixed header
      const highY = () => {
        const hdr = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--hdr-h')) || 76;
        return Math.max(-window.innerHeight * .12, hdr + 12 + cards[0].offsetHeight / 2 - window.innerHeight / 2);
      };
      cards.forEach((c, i) => {
        const at = .35 + i * STEP;
        // x runs straight across while y goes up and then down: together that draws the arc.
        // The card arrives turned away in 3D, straightens out at the top of the arc and turns away again as it leaves.
        // autoAlpha: a card that is not in flight is visibility:hidden, so its frosted backdrop costs nothing
        tl.fromTo(c, { x: farX, y: lowY, autoAlpha: 0 }, { x: () => -farX(), duration: 2, ease: 'none' }, at)
          .fromTo(c, { y: lowY }, { y: highY, duration: 1, ease: 'sine.inOut' }, at)
          .to(c, { y: lowY, duration: 1, ease: 'sine.inOut' }, at + 1)
          .fromTo(c, { rotationY: 30, rotationZ: 9 }, { rotationY: 0, rotationZ: 0, duration: 1, ease: 'sine.inOut' }, at)
          .to(c, { rotationY: -30, rotationZ: -9, duration: 1, ease: 'sine.inOut' }, at + 1)
          .fromTo(c, { scale: .84 }, { scale: 1, duration: 1, ease: 'sine.out' }, at)
          .to(c, { scale: .88, duration: 1, ease: 'sine.in' }, at + 1)
          .fromTo(c, { autoAlpha: 0 }, { autoAlpha: 1, duration: .22, ease: 'none' }, at)
          .to(c, { autoAlpha: 0, duration: .22, ease: 'none' }, at + 1.78);
      });
      tl.to({}, { duration: .25 }); // a short tail, so a fast scroll can never leave the last card half-visible
      ...
      const onSelect = () => {
        ...
        window.scrollTo(0, st.start + (st.end - st.start) * ((1.35 + i * STEP) / tl.duration()));
      };
